...
example {u v x y A B : ℝ} (h1 : 0 < A) (h2 : A ≤ 1) (h3 : 1 ≤ B) (h4 : x ≤ B)
  ...
  calc
    u * y + v * x + u * v
      ≤ u * B + v * B + u * v := by rel [h4, h5, h6, h7]
    _ ≤ A * B + A * B + A * v := by rel [h3, h7, h8, h9]
    _ ≤ A * B + A * B + 1 * v := by rel [h2]
    _ ≤ A * B + A * B + B * v := by rel [h3]
    _ < A * B + A * B + B * A := by rel [h1, h9]
    _ = 3 * A * B := by ring
...
example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 :=
  calc
    n ^ 4 - 2 * n ^ 2 = n * n * n^2 - 2 * n * n := by ring
    _ = n^2 * (n * n - 2) := by ring
    _ ≥ n^2 * (10 * n - 2) := by rel [hn]
    _ = 10 * n^3 - 2 * n^2 := by ring
    _ = 10 * n^3 - 10 * n^2 + 8 * n^2 := by ring
    _ ≥ 10 * n^3 - 10 * n^2 := by extra
    _ ≥ 10 * n^3 - n * n^2 := by rel [hn]
